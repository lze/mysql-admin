...
%EXPORT_TAGS = (
    ...
    'lze' =>
      [qw/$ACCEPT_LANGUAGE translate init session createSession $m_hrParams clearSession $m_qy include sessionValidity includeAction/],
);
...
MySQL::Admin is a Web-frontend like phpmyadmin.

Install it and open in your Webbrowser /index.html

The Project Website is http:sourceforge.net/p/lindnerei.

The Documentation is only usefule if you try to change the sourcecode.


MySQL::Admin is a CGI subclass, 


This Module is mainly written for L<MySQL::Admin::GUI>.
...
export tags:
lze: $ACCEPT_LANGUAGE translate init session createSession $m_hrParams clearSession $m_qy include sessionValidity includeAction
...
sub clearSession
{
    foreach my $ua (keys %{$m_qy}) {
	  ...
	  my $t = $m_qy->{$ua}{$entry}{timestamp} ? time() - $m_qy->{$ua}{$entry}{timestamp} : time();
	  $hold = defined $m_qy->{$ua}{$entry}{validity} ? defined $m_qy->{$ua}{$entry}{validity} : $hold;
