...
@ISA                           = qw(Exporter);
$MySQL::Admin::Config::VERSION = '0.86';
...
sub saveConfig
{
    ...
    if ($rsas) {
        open $fh, ">$rsas.bak" or warn "$/MySQL::Admin::Config::saveConfig$/ $! $/ File: $rsas $/Caller: " . caller() . $/;
        flock $fh, 2;
        seek $fh, 0, 0;
        truncate $fh, 0;
        print $fh $content;
    ...
    if (-e "$rsas.bak") {
        rename "$rsas.bak", $rsas or warn "$/MySQL::Admin::Config::saveConfig$/ $! $/ File: $rsas $/Caller: " . caller() . $/;
        do $rsas;
    }
}
