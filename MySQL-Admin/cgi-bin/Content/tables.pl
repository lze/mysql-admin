...
sub ShowNewTable
{
    ...
    $m_sContent .= qq(
    <form onsubmit="setAll();submitForm(this,'$m_sAction','$m_sTitle');return false;" method="get" enctype="multipart/form-data">
    ...
    <td class="caption3 "><img src="style/$m_sStyle/buttons/primary.png" title=").translate('AddPrimaryKey').qq(" /></td>
    ...
    if ( defined $tbl and defined $session )
    {
	    ...
	    <td class="forms">|. GetTypes( $type, $m_hrParams->{rows}[$row]{Type},$tbl, $fie1d ,\$set ) . qq{</td>
	    ...
	    <td class="forms">
	    <input type="checkbox" class="checkbox" } . ($prim ? 'checked="checked"' : '') . qq { name="$m_hrParams->{rows}[$row]{Primary}"/>
	    </td><td class="forms">
	    }.(
	    $bFulltext
	    ...
	    ( $bUnique ? qq|<input type="checkbox" name="$m_hrParams->{rows}[$row]{Unique}" title="Unique" checked="checked"/>| : qq|<input type="checkbox" name="$m_hrParams->{rows}[$row]{Unique}" title="Unique"/> | ).q{</td></tr>};
    ...
    if (defined $tbl and defined $session) {
        ...
        my $indexes;
         for ( my $row = 0; $row <= $#{$m_hrParams->{rows}}; $row++ ) {
            ...
            my $length = param($m_hrParams->{rows}[$row]{Length});
            my $fie1d    = param($m_hrParams->{rows}[$row]{Field});
            $type = $type =~ /BLOB|MEDIUMBLOB|LONGBLOB|TINYBLOB|TIMESTAMP/ ? $type : $length ? $type . "($length)" : $type;
...
sub ShowDumpTable
{
    ...
    $m_sContent .= '<div class="dumpBox">';
    $m_sContent .= qq(<textarea class="dumpTextarea">);
    DumpTable( $tbl );
...
sub ShowDumpDatabase
{
    ...
    $m_sContent .= qq(<div align="left" class="dumpBox" style="width:100%;padding-top:5px;"><textarea style="width:100%;height:800px;overflow:auto;">);
    DumpDatabase($m_sCurrentDb);
    $m_sContent .= qq(</textarea></div>);
}
...
sub getLink
{
  ...
  {
    return qq|<a style="color:red;" onclick="requestURI('$ENV{SCRIPT_NAME}?action=searchHelpTopic&topic=$_[0]','searchHelpTopic','searchHelpTopic')" title="$m_hrLng->{$ACCEPT_LANGUAGE}{search}">$_[0]</a>|;    
  }
}
...
sub searchHelpTopic
{
    ...
    my $sql = qq|SELECT help_topic.name, help_topic.url, help_topic.description, help_topic.example  FROM help_topic join help_relation on (help_topic.help_topic_id = help_relation.help_topic_id ) JOIN help_keyword ON (help_keyword.help_keyword_id = help_relation.help_keyword_id) where help_keyword.name like ?;|;
    my @aoh = $m_oDatabase->fetch_AoH( $sql,$topic );
    my $sContent ='';
    $m_sContent .='<div class="ShowTables" style="margin-top:4%;">';
    for ( my $j = 0; $j < $#aoh; $j++ )
    {
      $m_sContent .= qq|<a onclick="\$('hid$j').scrollIntoView();">$aoh[$j]->{name}</a><br/>|;
      $sContent   .= qq|<div class="ShowTables"><a id="hid$j" href="$aoh[$j]->{url}">$aoh[$j]->{name}</a><a onclick="hide('Example$j');visible('Description$j');">Description</a> |;
      $sContent   .= qq|<a onclick="visible('Example$j');hide('Description$j');">Example</a><pre id="Example$j" style="display:none;">$aoh[$j]->{example}</pre>| if $aoh[$j]->{example};
      $sContent   .= qq|<pre id="Description$j" >$aoh[$j]->{description}</pre></div>|;
    }
    $m_sContent .= qq|<a href="http://google.com?q=$topic">Google($topic)</a>|;
    $m_sContent .= '</div><div  id="topUp" onclick="scrollToTop()" title="top">^</div>'.$sContent ;
    ChangeDb(
      {
	name     => $m_sCurrentDb,
	host     => $m_sCurrentHost,
    ...
    foreach my $s ( @statements )
    {
	    ...
	    if ( $m_dbh->errstr) {
	      ...
	      $ret = 0;
            ...
            $RIBBONCONTENT .= '<div class="ExecSql" align="center">' if $showSql and $rows_affected > 0;
            if ( $showSql ) {
                if ( $rows_affected > 0 ) {
		    ...
		    while ( my @rows = $sth->fetchrow_array()){
		      my %columns;
		      $RIBBONCONTENT .= "<table><tr>";
		      for (my $i = 0 ; $i < $sth->{NUM_OF_FIELDS} ; $i++ ) {
			my $nType = $sth->{TYPE}->[$i];
			my $type  = $types{$nType};
			$columns{$sth->{NAME}->[$i]} = $i; 
			$RIBBONCONTENT .= "<td class=\"caption3\">$sth->{NAME}->[$i] ($type)</td>";
		      }
		      #$parameter{id} = "ExecSql$id";
		      $RIBBONCONTENT .= qq|</tr><tr>|;
		      for (my $i = 0 ; $i <= $#rows; $i++ ) {
			unless ( utf8::is_utf8(  $rows[$i] ) ){
			  utf8::decode(  $rows[$i] );
		      }
		      $RIBBONCONTENT .= qq|<td class="values" >|. encode_entities( $rows[$i] ) . '</td>'; 
		      }
		      $RIBBONCONTENT .= qq|</tr>|;
                        if ( $m_oDatabase->tableExists( $table ) ) {
                            ...
                            <a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=EditEntry&table=$table&$eid','$m_sAction','$m_sTitle');">$tredit</a>
                            <a href="javascript:confirm2('$trdelete ?',requestURI,'$ENV{SCRIPT_NAME}?action=DeleteEntry&table=$table&$eid','$m_sAction','$m_sTitle')">$trdelete</a></td>
                            </tr>|;
                        }
                        $id++;
                        $RIBBONCONTENT .= '</table><hr/>' ;
                    }
		  }
	      }
            }
        ...
        $ret = 0 if $@;
        $id2++;
        $RIBBONCONTENT .= '</div>' if $showSql and $rows_affected > 0;
    }
...
sub ShowTable
{
        ...
        my $menu = (
            ...
            ? div(
                ...
                translate( 'links_pro_page' ) . ' | '
                    . a(
                    {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl&links_pro_page=10&von=$m_nStart&orderBy=$field&desc=$state','ShowTable','ShowTable')",
                    ...
                    ? ' | '
                        . a(
                        {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl&links_pro_page=20&von=$m_nStart&orderBy=$field&desc=$state','ShowTable')",
                    ...
                    ? ' | '
                        . a(
                        {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable;&table=$tbl&links_pro_page=30&von=$m_nStart&orderBy=$field&desc=$state','ShowTable','ShowTable')",
                    ...
                    ? ' | '
                        . a(
                        {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl&links_pro_page=100&von=$m_nStart&orderBy=$field&desc=$state','ShowTable','ShowTable')",
                    ...
                    ? ' | '
                        . a(
                        {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl&links_pro_page=1000&von=$m_nStart&orderBy=$field&desc=$state','ShowTable','ShowTable')",
    ...
    my $toolbar = a(#todo
        {   class       =>  'toolbarButton',
            onclick     => "\$('popupContent1').style.left='5%';\$('popupContent1').style.width='90%';showPopup('NewEntry');",
            onmouseover => "window.status='" . translate( 'NewEntry' ) . "'",
    ...
    $toolbar .= a(
        {   class =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=EditTable&table=$tbl','EditTable','EditTable')",
    ...
    $toolbar .= a(
        {   class =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=ShowTableDetails&table=$tbl','ShowTableDetails','ShowTableDetails')",
    ...
    $toolbar .= a(
        {   class =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=ShowDumpTable&table=$tbl','ShowDumpTable','ShowDumpTable')",
    ...
    $toolbar .= a(
        {   class    =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=AnalyzeTable&table=$tbl','AnalyzeTable','AnalyzeTable')",
    ...
    $toolbar .= a(
        {   class    =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=OptimizeTable&table=$tbl','OptimizeTable','OptimizeTable')",
    ...
    $toolbar .= a(
        {   class    =>  'toolbarButton',
            onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=RepairTable&table=$tbl','RepairTable','RepairTable')",
    ...
    $toolbar .= a(
        {   class   =>  'toolbarButton',
            ...
            onclick => "confirm2('" . translate( 'truncate' ) . "?',requestURI,'$ENV{SCRIPT_NAME}?action=TruncateTable&table=$tbl','TruncateTable','TruncateTable')",
        },
        translate( 'truncate' )
    );#11
    $toolbar .= a(
        {   class   =>  'toolbarButton',
            ...
            onclick => "confirm2('" . translate( 'delete' ) . "?',requestURI,'$ENV{SCRIPT_NAME}?action=DropTable&table=$tbl','DropTable','DropTable')",
        },
        translate( 'Delete' )
    );#12
    ...
    <tr class="captionRadius" onmouseout="toolbarOut()" onmouseover="toolbarOver()">
    <td class="captionRadius" colspan="$rws2">$tbl</td></tr>
    <tr><td onmouseout="toolbarOut()" onmouseover="toolbarOver()" colspan="$rws2" id="toolbar" class="toolbar"><div id="toolbarcontent" style="display:none" class="toolbarcontent">$toolbar</div></td></tr>
    );
    $m_sContent .= qq(<tr><td onmouseout="toolbarOver()" onmouseover="toolbarOver()" colspan="$rws2"  id="toolbar2" class="toolbar2"><div id="toolbarcontent2" style="display:none" class="toolbarcontent">
    <div class="makePages">$PAGES</div>
    <div class="pagePerSite">$menu</div>
    </div> </td></tr>) if $count  > 10;

    $m_sContent .=qq(<tr class="caption2"><td class="caption2 checkbox"></td>);

	for ( my $i = 0; $i <= $rows; $i++ )
	  {
	    $m_sContent .= qq|<td class="caption2">|;
	    $m_sContent .= a(
		{
			class => $caption[$i]->{'Field'} eq $field
			? 'currentLink'
			: 'link',
			href => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl&links_pro_page=$lpp&von=$m_nStart&orderBy=$caption[$i]->{'Field'}&desc="
			. (
        ...
        $m_sContent .= '<td class="caption2 checkbox"></td>';
        my @p_key    = $m_oDatabase->GetPrimaryKey( $tbl );
        my $trdelete = translate( 'delete' );
        my $tredit   = translate( 'EditEntry' );
        for ( my $i = 0; $i <= $#a; $i++ )
        {
            $m_sContent .= q|<tr onmouseover="closeToolbar()" >|;
            my $eid  = "";
            my $pkey = "";
            if ( $#p_key > 0 )
            {
            ...
            $m_sContent .= qq|<td class="checkbox"><input type="checkbox" name="markBox$i" class="markBox" value="$tbl/$pkey"/></td>|;
	      ...
	      {
		$headline = qq|<a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=downLoadFile&table=$tbl&blob=$caption[$j]->{Field}&pkey=$p_key[0]&pkeyValue=$pkey','downLoadFile','downLoadFile')">Download</a>|;
		$m_sContent .= '<td class="values">'.  $headline .'</td>';
	      }
	      else
	      {
		$headline = defined $a[$i]->{$caption[$j]->{Field}} ? $a[$i]->{$caption[$j]->{Field}} : '';
		$m_sContent .= '<td class="values">'. substr( $headline, 0, int( 120 / ( $rows > 0 ? $rows : 1 ) ) ) .'</td>';
	      }
            }
	    $m_sContent .= qq|<td class="values right"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=EditEntry&table=$tbl&$eid&von=$m_nStart&bis=$m_nEnd','EditEntry','EditEntry')"><img src="style/$m_sStyle/buttons/edit.png" title="$tredit"/></a><a onclick="confirm2('$trdelete ?',requestURI,'$ENV{SCRIPT_NAME}?action=DeleteEntry&table=$tbl&$eid&von=$m_nStart;&bis=$m_nEnd;','DeleteEntry','DeleteEntry')"><img src="style/$m_sStyle/buttons/delete.png" title="$trdelete"/></a></td></tr>|;
        }
        $m_sContent .= qq|<tr><td class="checkbox"><img src="style/$m_sStyle/buttons/feil.gif"/></td>|;
	...
	<option value="export">$export</option>
        ...
        $m_sContent .= qq|</td></tr></table></form>|;
    }
    else
    {
        ShowTables();
...
sub MultipleAction
{
    ...
    {
        ShowDbHeader( $tbl, 1, 'Export' );
        $m_sContent .= qq(<div class="dumpBox" style="padding-top:5px;width:100%;padding-right:2px;"><textarea style="width:100%;height:800px;overflow:auto;">);
    }
...
sub MultipleDbAction
{
    ...
    {
        ShowDbHeader( $m_sCurrentDb, 0, 'Export' );
        $m_sContent .= qq(<div align="left" class="dumpBox"><textarea style="width:100%;height:800px;">);
    }
    if ( $a eq 'exportDb' )
    {
        ShowDbHeader( $m_sCurrentDb, 0, 'Export' );
...
sub EditEntry
{
	...
	$RIBBONCONTENT .= qq(
	<form class="EditEntry" onsubmit="submitForm(this,'$m_sAction','$m_sTitle',false,'POST');return false;" method="get" enctype="multipart/form-data">
	...
	<td class="caption captionRight">Extra</td>
	  ...
	  SWITCH:{
	    ...
	    {
		  $RIBBONCONTENT .= qq(<tr><td class="values">$caption[$j]->{'Field'}</td><td class="values"><textarea name="tbl$caption[$j]->{'Field'}" >$a->{$caption[$j]->{'Field'}}</textarea></td><td class="values">$caption[$j]->{'Type'}</td><td>$caption[$j]->{'Null'}</td><td class="values">$caption[$j]->{'Key'}</td><td class="values">$caption[$j]->{'Default'}</td><td class="values">$caption[$j]->{'Extra'}</td></tr>);
		  last SWITCH;
	    }
	    if ( $caption[$j]->{'Type'} =~ /BLOB|LONGBLOB|MEDIUMBLOB|TINYBLOB/ )
	    {
...
sub downLoadFile
{
    ...
    $m_sAjax .= qq|<output id="errorMessage">|. $m_dbh->errstr.br().HighlightSQl( $sql ) .qq|</output>| if $m_dbh->errstr ;
    ...
    use Symbol;
    my $fh = gensym();
    open $fh, ">$m_hrSettings->{uploads}{path}/$pkeyValue.bak" or warn "tables::downLoadFile: $m_hrSettings->{uploads}{path}/$pkeyValue.bak $!";
    print $fh $rows[0];
    close $fh;

    my $trDownload = translate( 'download' );
    rename "$m_hrSettings->{uploads}{path}/$pkeyValue.bak", "$m_hrSettings->{uploads}{path}/$pkeyValue" or warn "tables::downLoadFile: $!";
    chmod  "$m_hrSettings->{uploads}{chmod}", "$m_hrSettings->{uploads}{path}/$pkeyValue"  if -e "$m_hrSettings->{uploads}{path}/$pkeyValue";
    $m_sContent .= qq|<a href="download/$pkeyValue" target="_blank">$trDownload</a>|;
    &ShowTable();
}
...
sub ShowNewEntry
{
	...
	my $save = translate('save');
	$m_sContent .= qq(<tr><td colspan="7" class="submit"><input type="submit" value="$save"/></td></tr></table></form>);
    }
    else
    {
        ShowTables();
...
sub SaveEntry
{
    ...
    if ( $m_oDatabase->tableExists( $tbl ) )
    {
      ...
      if( $columns[$i]->{Type} eq 'blob' or $columns[$i]->{Type} eq 'longblob' or $columns[$i]->{Type} eq 'tinyblob' or  $columns[$i]->{Type} eq 'mediumblob')
      {
	push @uploads,
	{
	  table 		=> $tbl,
...
# 	  $primaryKeyValue = $pa if $params[$i] eq $p_key[0];
	  if ( $#p_key > 0 )
	  {
	    for ( my $j = 0; $j < $#p_key; $j++ )
	    {
	      $eid .= "$p_key[$j] = " . $m_oDatabase->quote( $pa ) . ' && ' if  $params[$i] eq $p_key[$j] ;
	    }
	      $eid .= "$p_key[$#p_key] = " . $m_oDatabase->quote( $pa ) if $params[$i] eq $p_key[$#p_key];
	  }
	  else
	  {
	  $eid .= "$p_key[0] = " . $m_oDatabase->quote( $primaryKeyValue ) if $params[$i] eq $p_key[0];
	  }
...
sub ShowTables
{
    ...
    $toolbar .= a(
      {   class    =>  'toolbarButton',
	  onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=ShowDumpDatabase','ShowDumpDatabase','ShowDumpDatabase');",
	...
	. (
	$#a > 10
	? a({   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=10&von=$m_nStart&orderBy=$orderby&desc=$state','ShowTables','ShowTables')",
		class => $lpp == 10 ? 'menuLink2' : 'menuLink3'
	    },
	    '10'
	    )
	...
	? '&#160;'
	    . a(
	    {   
		href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=20&von=$m_nStart&orderBy=$orderby&desc=$state','ShowTables','ShowTables')",
		class => $lpp == 20 ? 'menuLink2' : 'menuLink3'
	    },
	    '20'
	    )
	...
	? '&#160;'
	    . a(
	    {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=30&von=$m_nStart&orderBy=$orderby&desc=$state','ShowTables','ShowTables')",
		class => $lpp == 30 ? 'menuLink2' : 'menuLink3'
	    },
	    '30'
	    )
	...
	? '&#160;'
	    . a(
	    {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=100&von=$m_nStart&orderBy=$orderby&desc=$state','ShowTables','ShowTables')",
		class => $lpp == 100 ? 'menuLink2' : 'menuLink3'
	    },
	    '100'
	    )
	...
	? '&#160;'
	    . a(
	    {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=1000&von=$m_nStart&orderBy=$orderby&desc=$state','ShowTables','ShowTables')",
		class => $lpp == 1000 ? 'menuLink2' : 'menuLink3'
	    },
	    '1000'
	    )
	...
	<table class="ShowTables" id="toolbarTable">
	<tr class="captionRadius" onmouseout="toolbarOut()" onmouseover="toolbarOver()"><td class="captionRadius" colspan="7">$m_sCurrentDb</td></tr>
	<tr><td onmouseout="toolbarOut()" onmouseover="toolbarOver()" colspan="7" id="toolbar" class="toolbar"><div id="toolbarcontent" style="display:none" class="toolbarcontent">$toolbar</div></td></tr>);

	$m_sContent .= qq(<tr><td onmouseout="toolbarOut()" onmouseover="toolbarOver()" colspan="7" id="toolbar2" class="toolbar2"><div id="toolbarcontent2" style="display:none"  class="toolbarcontent"><div class="makePages">$PAGES</div><div class="pagePerSite">$menu</div></div></td></tr>) if $#a  > $lpp;
	$m_sContent .= qq(<tr class="caption2"><td class="caption2 checkbox"></td><td class="caption2">)
        . qq(<a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=$lpp&von=$m_nStart&orderBy=Name&desc=$nstate','$m_sAction','$m_sTitle')">Name</a>)
        . ( $orderby eq 'Name' ? $state ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png" />| : qq|&#160;<img src="style/$m_sStyle/buttons/down.png"   />| : '' )
        . qq(</td><td class="caption2"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=$lpp&von=$m_nStart&orderBy=Rows&desc=$nstate','$m_sAction','$m_sTitle')">$m_hrLng->{$ACCEPT_LANGUAGE}{rows}</a>)
        . (
          $orderby eq 'Rows'
        ? $state
                ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png" />|
        ...
        . qq(<a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=$lpp&von=$m_nStart&orderBy=Type&desc=$nstate','$m_sAction','$m_sTitle')">$m_hrLng->{$ACCEPT_LANGUAGE}{type}</a>)
        . (
          $orderby eq 'Type' ? $state ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png"/>| : qq|&#160;<img src="style/$m_sStyle/buttons/down.png"/>| : ''
        )
        . q(</td><td class="caption2"> )
        . qq(<a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&db=$m_sCurrentDb&von=$m_nStart&bis=$m_nEnd&links_pro_page=$lpp&von=$m_nStart&orderBy=Data_length&desc=$nstate','$m_sAction','$m_sTitle')">$m_hrLng->{$ACCEPT_LANGUAGE}{size}&#160;(kb)</a>)
        . (
      ...
      $m_sContent .= qq(
      <tr onmouseover="closeToolbar()">
      <td class="checkbox" width="5%"><input type="checkbox" name="markBox$i" class="markBox" value="$a[$i]->{Name}" /></td>
      <td class="values" width="10%"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$a[$i]->{Name}&desc=0','showTable','showTable')">$a[$i]->{Name}</a></td>
      ...
      <img src="style/$m_sStyle/buttons/delete.png" title="$trdelete"  style="cursor:pointer;" onclick="confirm2(' $trdelete?',requestURI,'$ENV{SCRIPT_NAME}?action=DropTable&table=$a[$i]->{Name}&$eid','$m_sAction','$m_sTitle')"/>
      <img src="style/$m_sStyle/buttons/edit.png"  alt="$change" style="cursor:pointer;" title="$change" onclick="requestURI('$ENV{SCRIPT_NAME}?action=EditTable&table=$a[$i]->{Name}&$eid','EditTable','EditTable')"/>
      <img src="style/$m_sStyle/buttons/details.png"  style="cursor:pointer;" alt="Details" title="Details" onclick="requestURI('$ENV{SCRIPT_NAME}?action=ShowTableDetails&table=$a[$i]->{Name}&$eid','ShowTableDetails','ShowTableDetails')"/></td>
      </tr>
      );
    }
    my $delete   = translate( 'delete' );
...
sub ShowTableDetails
{
  ...
  for ( my $i = 0; $i <= $#a; $i++ )
  {
	  ...
	  {
	      $m_sContent .= qq(<tr class="values"><td class="values">$key</td><td class="values">$a[$i]->{$key}</td></tr>);
	  }
      }
  }
  $m_sContent .= '</table>';
...
sub EditTable
{
	...
	my $toolbar = a(
	      {   class =>  'toolbarButton',
		  onclick  => "requestURI('$ENV{SCRIPT_NAME}?action=ShowTable&table=$tbl','ShowTable','ShowTable')",
	...
	$toolbar .= a(
	    {   
		class =>  'toolbarButton',
		onclick  => "showPopup('ChangeCharset')",
	...
	$toolbar .= a(
	    {   
		class =>  'toolbarButton',
		onclick  => "showPopup('ChangeAutoInCrementValue')",
	...
	$toolbar .= a(
	    {   
		class =>  'toolbarButton',
		onclick  => "showPopup('ChangeEngine')",
	...
	$toolbar .= a(
	    {   class =>  'toolbarButton',
		onclick  => "showPopup('ShowEditIndex')",
	...
	<tr><td onmouseout="toolbarOut()" onmouseover="toolbarOver()" colspan="14" id="toolbar" class="toolbar"><div id="toolbarcontent" style="display:none" class="toolbarcontent">$toolbar</div></td></tr>
	);
	$m_sContent .= qq|
	<tr class="caption2">
	...
	<td class="caption2 checkbox"></td>
		...
		if($indexes[$j]->{field} eq $field){           
		  ...
		  if($indexes[$j]->{type} eq 'FULLTEXT KEY'){
            ...
            #>>
            $m_sContent .= qq|
              <tr class="values" onmouseover="closeToolbar()"> 
              ...
              <td class="values">|. $m_oDatabase->GetExtra( $caption[$j]->{'Extra'}, $sUniqueExtra ) .'</td>
              <td class="values">'. $m_oDatabase->GetColumnCollation( $tbl, $field, $sUniqueCollation ) . qq{</td>
		...
		(
              ...
              <a href="javascript:void(0)" onclick="confirm2('Delete $field',requestURI,'$ENV{SCRIPT_NAME}?action=DropCol;&table=$tbl&column=$field','$m_sAction','$m_sTitle')" title="$m_hrLng->{$ACCEPT_LANGUAGE}{DropColumn} $field" ><img src="style/$m_sStyle/buttons/delete.png"  align="left" /></a>
              </td>
              </tr>
	    };
            $vars{rows}{$field} = {
	...
	if($#index >= 0 ){ 
       ...
       <td class="values"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowEditIndex;&tbl=$tbl&index=$_->{'Key_name'}&editIndexOlp145656=1;','$m_sAction','$m_sTitle')" title="Edit Index $_->{'Key_name'}"><img src="style/$m_sStyle/buttons/edit.png" alt="Edit Index $_->{'Key_name'}" /></a></td>
       <td class="values"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=DropIndex;&table=$tbl&index=$_->{'Key_name'};&constraint=$_->{'Column_name'}','$m_sAction','$m_sTitle')" title="Drop Index $_->{'Key_name'}"><img src="style/$m_sStyle/buttons/delete.png"  align="left" /></a></td>
       </tr>| foreach @index;
        $m_sContent .= '</table>';
        }
        #Index bearbeiten
	...
	<div class="dbForm">
	<form class="dbForm" onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" method="get" enctype="multipart/form-data">
	...
	<div class="dbForm">
	<form class="dbForm" onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" enctype="multipart/form-data" accept-charset="utf-8">
	...
	<div class="dbForm"><form class="dbForm" onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" method="get" enctype="multipart/form-data">
	...
	<input type="text" value="|. $m_oDatabase->GetAutoIncrementValue( $tbl ) . qq|" name="AUTO_INCREMENT"/>
	...
	<div class="dbForm"><form class="dbForm" onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" method="get" enctype="multipart/form-data">
	|. $m_oDatabase->GetCharset( 'charset', $tbl ) . qq|<br/>
...
sub ShowEditIndex
{
    ...
    my @indexes = $m_oDatabase->getIndex($tbl);
    my ($foreignFields,$foreignReferences,$foreignConstraint,$foreignTable,$sForeignUpdate,$sForeignDelete,$bForeign,$sForeignKeyname, $bPrimary,$bIndex, $bUnique, $bFulltext,$sPrimaryKeyname,$sFulltextKeyname,$sUniqueKeyname,$sKeyname) = 0*16;
    for( my $j = 0; $j <= $#indexes; $j++ ){
    ...
    $RIBBONCONTENT .= qq|
    <div class="ShowTables" style="margin-top:4%;">
    <form  onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" method="get" enctype="multipart/form-data">
    <table align="center">
    <tr><td class="caption3">$sField</td>
    <td class="caption3">$sSize</td>
    <td class="caption3 foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|>Reference Table</td>
    <td class="caption3 foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|>Reference Field</td></tr>|;
    ...
    if ( $bEditIndex ) 
    {
        ...
        for ( my $i = 0; $i <= $#index; $i++ ) {
            ...
            $RIBBONCONTENT .= qq|<tr><td class="values">$columns</td><td class="values"><input type="text" value="$index[$i]->{Sub_part}" name="$uSize" style="width:40px;"/></td>|;

            my @tables  = $m_oDatabase->fetch_array( "show Tables;" );
	    $RIBBONCONTENT .= qq|<td class="values foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|>
		...
		$RIBBONCONTENT .=qq(<option value="$tables[$j]") . ( $tables[$j]  eq  $foreignTable  ? 'selected="selected"' : '' ) . qq( class="table">$tables[$j]</option>);
	    }
	    ...
	    for( my $j = 0; $j <= $#tables; $j++ )
	    {
	      next if $tables[$j] eq $tbl;
	      my $table = $tables[$j];
	      push @current, $cI.$tables[$j] if(( $tables[$j]  eq  $foreignTable ) or ( !$bForeign and $j == 0) );
	      $table = $m_dbh->quote_identifier( $table );
	      my @tables2 = $m_oDatabase->fetch_AoH( "show columns from $table" );
	      $RIBBONCONTENT .= qq|<select id="b$cI$tables[$j]" name="table$i$tables[$j]" size="1" style="| . (  ($tables[$j] eq $foreignTable) or ( !$bForeign and $j == 0)? '' : 'display:none;' ) . qq|">|;

	      for( my $k = 0; $k <= $#tables2; $k++ )
	      {
		my $bcurrent    = 0;
		for(my $l = 0;$l <= $#{$foreignFields};$l++){
		  if($foreignReferences->[$l] eq $tables2[$k]->{'Field'} and $index[$i]->{Column_name} eq $foreignFields->[$l] )
		  {
		    $bcurrent       =   1;
		  }
		}
		$RIBBONCONTENT .= '<option ' . ( $bcurrent  ? 'selected="selected"' : '' ) . qq(value="$tables2[$k]->{'Field'}" class="table">$tables2[$k]->{'Field'}</option>);
	      }
	...
	my @tables  = $m_oDatabase->fetch_array( "show Tables;" );
	$RIBBONCONTENT .= qq|<td class="values foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|><select name="tablelist$_" onchange="DisplayTables($_,'b$_'+this.options[this.options.selectedIndex].value)" id="tablelist"  size="1">|;
	for ( my $j = 0; $j <= $#tables; $j++ )
	{
	    next if $tables[$j]  eq $tbl;
	...
	for ( my $j = 0; $j <= $#tables; $j++ )
	{
	  ...
	  $table = $m_dbh->quote_identifier( $table );
	  my @tables2 = $m_oDatabase->fetch_AoH( "show columns from $table" );
	  $RIBBONCONTENT .= qq|<select id="b$_$tables[$j]" name="table$_$tables[$j]" size="1" style="| . ( $j == 0 ? '' : 'display:none;' ) . qq|">|;

	  for ( my $k = 0; $k <= $#tables2; $k++ )
	  {
	    $RIBBONCONTENT .= '<option '.( $k == 0  ? 'selected="selected"' : '' ).qq(value="$tables2[$k]->{'Field'}" class="table">$tables2[$k]->{'Field'}</option>);
	  }
	  $RIBBONCONTENT .= '</select>';
	}
	$RIBBONCONTENT .= qq|</tr>|;
    ...
    <td class="caption3 foreign"|.( $bForeign  ? '' : 'style="display:none"'  ). qq|>ON UPDATE</td>
    ...
    <option value="PRIMARY"|.(     $bPrimary  ? 'selected="selected"' : '' ). qq|>PRIMARY</option>
    <option value="INDEX"|.(       $bIndex    ? 'selected="selected"' : '' ) .qq|>INDEX</option>
    <option value="UNIQUE"|.(      $bUnique   ? 'selected="selected"' : '' ) .qq|>UNIQUE</option>
    <option value="FULLTEXT" |.(   $bFulltext ? 'selected="selected"' : '' ). qq|>FULLTEXT</option>
    <option value="FOREIGN KEY"|.( $bForeign  ? 'selected="selected"' : '' ). qq|>FOREIGN</option>
    </select>
    </td>
    <td class="values foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|>
    <select name="$sUniqueUpdate">
    <option value="NO ACTION">NO ACTION</option>
    <option value="CASCADE"|.($sForeignUpdate eq 'CASCADE' ? 'selected="selected"' : '' ) . qq|>CASCADE</option>
    <option value="SET DEFAULT"|.($sForeignUpdate eq 'SET DEFAULT' ? 'selected="selected"' : '' ) . qq|>SET DEFAULT</option>
    </select>
    </td>
    <td class="values foreign" |.( $bForeign  ? '' : 'style="display:none"'  ). qq|>
    <select name="$sUniqueDelete">
    <option value="NO ACTION">NO ACTION</option>
    <option value="CASCADE"|.($sForeignDelete ? 'selected="selected"' : '' ) . qq|>CASCADE</option>
    <option value="SET DEFAULT"|.($sForeignDelete eq 'SET DEFAULT' ? 'selected="selected"' : '' ) . qq|>SET DEFAULT</option>
    </select>
    </td>
    </tr>
...
sub SaveEditTable
{
    ...
    if (defined $tbl and defined $session) {
	    ...
	    if($sfulltext and  $fulltext ne  'on'){
	      $m_oDatabase->void("ALTER TABLE $tbl2 drop index `$sfulltext`") if defined $sfulltext;
	    ...
	    $default =
	      ...
	      : '';
            ...
            if ( $collation) {
                $sql .= " CHARACTER SET $character_set COLLATE $collation" unless $character_set eq 'binary' or $collation eq 'NULL';
            }
            $sql .= " $attrs";
            $sql .= " $null ";
...
sub ShowDbHeader
{
    ...
    my $selected = shift;
    my $current  = shift;
    $m_sContent .= qq|<div id="NewEntry" style="display:none;">|;
	...
	<label for="name" class="caption">| . translate( 'CreateDatabase' ) . qq|</label>
	<input type="text" name="name" data-regexp="|.'/^.{1,100}$/'.qq|" data-error="$wrong_database_name" data-right="$right_database_name">
	<div align="right"><input type="submit" name="submit" value="| . translate( 'create' ) . qq|"/></div>
	...
	. translate( 'user' ) .
	q(</label>
	<input type="text" name="name" data-regexp=").'/^\w{4,100}$/'.qq(" data-error="$wrong_username_text" data-right="$right_username_text"/>
	...
	<input type="text" name="count" id="fields4tbl" data-regexp="/\d{1,3}/" data-error="\d{1,3}"  data-right="Ok"/>|.qq|
...
sub _insertTables{
    ...
    $list  .= qq|<select onsubmit="return false;" id="selKeyword" class="keyWords" size="10" onkeypress="var keyCode = event.keyCode ? event.keyCode :event.charCode ? event.charCode :event.which;if (keyCode != 13) return;insertAtCursorPosition(this.options[this.options.selectedIndex].value);return false;" style="display:none;">|;
    for ( my $i = 0; $i <= $#tables; $i++ )
    {
...
sub ShowUsers{
    ...
    <table class="ShowTables" id="toolbarTable">
    <tr class="captionRadius" onmouseout="toolbarOut()" onmouseover="toolbarOver(toolbar)">
      <td class="captionRadius" colspan="7">).translate('ShowUsers').qq(</td>
    </tr>
    <tr>
    <td onmouseout="toolbarOut()" onmouseover="toolbarOver()" colspan="8" id="toolbar" class="toolbar"><div id="toolbarcontent" class="toolbarcontent">$toolbar</div>
    </td>
    </tr>
    <tr>
    <td class="caption2 checkbox"></td>
	...
	<td class="values"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowRights&user=$a[$i]->{User}&host=$a[$i]->{Host};','$m_sAction','$m_sTitle')">$a[$i]->{User}</a></td>
	<td class="values">$a[$i]->{Host}</td>
	<td class="values">$sRights</td>
	<td class="values"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowRights&user=$a[$i]->{User}&host=$a[$i]->{Host};','$m_sAction','$m_sTitle')"><img src="style/$m_sStyle/buttons/edit.png"  alt="$change" title="$change"/></a></td>
	<td class="values right"><a href="javascript:void(0)" onclick="confirm2(' $trdelete?',requestURI,'$ENV{SCRIPT_NAME}?action=DeleteUser&table=mysql&user=$a[$i]->{User}&host=$a[$i]->{Host};','$m_sAction','$m_sTitle')"><img src="style/$m_sStyle/buttons/delete.png" title="$trdelete" /></a></td>
	</tr>
	);
    }
    my $delete   = translate( 'delete' );
...
sub ShowRights
{
    ...
    <tr>
      <td class="values nobr"><input type="checkbox" class="markBox" name="marksBox$UNIQUE_UPDATE" |. ( HasRight( 'update' ) ? 'checked="checked"' : "" ) . qq| />UPDATE</td>
    ...
    <tr>
      ...
      <td class="values" >|. GetDatabases( $UNIQUE_DB ).qq|</td>
      <td class="values" >|. GetTables( $UNIQUE_TBL ).qq|</td>
...
sub ShowDatabases
{
    ...
    for ( my $i = 0; $i <= $#a; $i++ ) {
        ...
        for ( my $j = 0; $j <= $#b; $j++ ) {#todo
        
            $kb += ($b[$j]->{Index_length} and $b[$i]->{Data_length})
		? $b[$j]->{Index_length} + $b[$i]->{Data_length} :
		  $b[$j]->{Index_length}? $b[$j]->{Index_length} : $b[$i]->{Data_length}  ? $b[$i]->{Data_length}: 0;
        }
    ...
    my $menu = translate( 'links_pro_page' ) . ' | '
            . (
            $#a > 10
            ? a({   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=10&von=$m_nStart&orderBy=$orderby&desc=$state','ShowDatabases','ShowDatabases')",
                    class => $lpp == 10 ? 'menuLink2' : 'menuLink3'
                },
                '10'
                )
            ...
            ? '&#160;'
                . a(
                {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=20&von=$m_nStart&orderBy=$orderby&desc=$state','ShowDatabases','ShowDatabases')",
                    class => $lpp == 20 ? 'menuLink2' : 'menuLink3'
                },
                '20'
                )
            ...
            ? '&#160;'
                . a(
                {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=30&von=$m_nStart&orderBy=$orderby&desc=$state','ShowDatabases','ShowDatabases')",
                    class => $lpp == 30 ? 'menuLink2' : 'menuLink3'
                },
                '30'
                )
            ...
            ? '&#160;'
                . a(
                {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=100&von=$m_nStart&orderBy=$orderby&desc=$state','ShowDatabases','ShowDatabases')",
                    class => $lpp == 100 ? 'menuLink2' : 'menuLink3'
                },
                '100'
                )
            ...
            ? '&#160;'
                . a(
                {   href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=1000&von=$m_nStart&orderBy=$orderby&desc=$state','ShowDatabases','ShowDatabases')",
                    class => $lpp == 1000 ? 'menuLink2' : 'menuLink3'
                },
                '1000'
                )
    ...
    $m_sContent .= qq(
              ...
              <input type="hidden" name="action" value="MultipleDbAction"/>
              <table class="ShowTables" id="toolbarTable"> 
              <tr><td colspan="7" class="captionRadius" onmouseout="toolbarOut()" onmouseover="toolbarOver()">$m_sCurrentHost</td></tr>
              <tr><td colspan="7" id="toolbar" class="toolbar" onmouseout="toolbarOut()" onmouseover="toolbarOver()"><div id="toolbarcontent" class="toolbarcontent"><a class="toolbarButton" onclick="showPopup('CreateDatabase')" class="link"  title="$createDatabase">$createDatabase</a><a id ="test" class="toolbarButton" onclick="showPopup('SqlSearch')" class="link"  title="$sqlSearch">$sqlSearch</a>
              <a class="toolbarButton" onclick="showSQLEditor()" class="link"  title="$execSql">$execSql</a>
              </div>
              </tr>);
   $m_sContent .= qq(<tr><td colspan="7" id="toolbar2" class="toolbar2" onmouseout="toolbarOut()" onmouseover="toolbarOver()"><div id="toolbarcontent2" class="toolbarcontent">
	      <div class="makePages">$PAGES</div>
   ...
   $m_sContent .= qq(</div>
              ...
              <td class="caption2 checkbox"></td>
              <td class="caption2"> )
        . qq|<a class="captionLink" href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=$lpp&von=$m_nStart&orderBy=Name&desc=$nstate','ShowDatabases','ShowDatabases')">$m_hrLng->{$ACCEPT_LANGUAGE}{name}</a>|
        . (
          $orderby eq "Name"
        ? $state
                ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png" />|
        ...
        . q(</td>
              <td class="caption2"> )
        . qq|<a class="captionLink" href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=$lpp&von=$m_nStart&orderBy=Tables&desc=$nstate','ShowDatabases','ShowDatabases')">$m_hrLng->{$ACCEPT_LANGUAGE}{showtables}</a>|
        . (
          $orderby eq "Tables"
        ? $state
                ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png" />|
        ...
        . qq|<a class="captionLink" href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowDatabases&links_pro_page=$lpp&von=$m_nStart&orderBy=Size&desc=$nstate','ShowDatabases','ShowDatabases')">$m_hrLng->{$ACCEPT_LANGUAGE}{size}&#160;(kb)</a>|
        . (
          $orderby eq "Size"
        ? $state
                ? qq|&#160;<img src="style/$m_sStyle/buttons/up.png" />|
    ...
    my $change     = translate( 'EditTable' );
    for ( my $i = $m_nStart; $i <= $end; $i++ ) {
        my $class =  $i == $m_nStart ? 'values':'values'; #todo ???? class 
        $m_sContent .= qq(
              <tr class="values" onmouseover="closeToolbar()">
              <td class="checkbox" width="5%"><input type="checkbox" name="markBox$i" class="markBox" value="$a[$i]->{Database}" /></td>
              <td class="$class" width="10%"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=ShowTables&m_ChangeCurrentDb=$a[$i]->{Database}&desc=0','ShowTables','ShowTables')">$a[$i]->{Database}</a></td>
              <td class="$class" width="10%">$a[$i]->{Tables}</td>
              <td class="$class" width="15%">$a[$i]->{Size}</td>
              <td class="$class right" width="*"><a href="javascript:void(0)" onclick="confirm2(' $trdelete?',requestURI,'$ENV{SCRIPT_NAME}?action=DropDatabase&db=$a[$i]->{Database}','DropDatabase','DropDatabase')"><img src="style/$m_sStyle/buttons/delete.png" title="$trdelete" /></a></td>
              </tr>
       );
    }
...
sub ShowProcesslist
{
    ...
    for ( my $i = 0; $i <= $#m_processlist;  $i++ ) {
      ...
      <td class="values nobr" align="left"><a href="javascript:requestURI('$ENV{SCRIPT_NAME}?action=killProzess&id=$m_processlist[$i]->{Id}','ShowProcesslist','ShowProcesslist')">Kill ($m_processlist[$i]->{Id})</a></td>   
      </tr>);
    }
    $m_sContent .= qq|</table></div>
    <script>setTimeout(function start(){if(window.location.search.match(/ShowProcesslist/))requestURI('$ENV{SCRIPT_NAME}?action=ShowProcesslist&reload','ShowProcesslist','ShowProcesslist',false,false,'GET',false)},400)</script>|
    unless $reload;
    if($#m_processlist  > 20)
    {
      shift @m_processlist;
...
sub searchForm
{
    ...
    my $form    = qq|
    <form class="dbForm" onsubmit="submitForm(this,'$m_sAction','$m_sTitle');return false;" name="search" method="get" accept-charset="UTF-8">
    <div align="center">
    ...
    for ( my $i = 0; $i <= $#tables; $i++ )
    {
      ...
      $table = $m_dbh->quote_identifier( $table );
      my @tables2 = $m_oDatabase->fetch_AoH( "show columns from $table" );
      $form .= qq|<select class="tablelist" multiple="multiple" id="a$tables[$i]" name="$tables[$i]" size="5" style="| . ( $i == 0 ? '' : 'display:none;' ) . qq|width:100%;">|;
    ...
    $limit : <input align="left" type="text" title="$ts" name="limit" value="10" style="width:80px;"/>
    <br/>
    </td>
    </tr>
    <tr>
...
sub searchDatabase
{
    ...
    my $limit   = param( 'limit' ) =~ /(\d+)/ ? $1 : 1000;#todo
	...
	my $request = '';
        ...
        {
            $columns[$j] = ( 
	    $hash ? $m_dbh->quote_identifier( $columns[$j]->{'Field'} ): $m_dbh->quote_identifier( $columns[$j] ) 
	);
        }
        my $col;
        if ( param( 'regexp' ) )
...
sub showProfile
{
  ...
  my $right_passwort_text = translate( 'right_passwort_text' );
  $m_sContent .= qq|
  <div class="ShowTables" style="margin-top:4%;padding:4%;width:92%;"><form name="changePassword" target="_parent" method="get" onsubmit="submitForm(this,'saveProfile','saveProfile');return false;">
  <label for="user">|.translate('name').qq(</label>
  <br/>
  <input type="text" id="user" data-regexp=").'/^\w{4,100}$/'.qq(" data-error="$wrong_username_text" data-right="$right_username_text" name="user" value="$hrUser->{user}">
  <br/>
  ...
  <br/>
  <br/>
  <input type="submit"  name="submit" value="|.translate('save').qq|"/>
...
sub saveProfile
{
    ...
    if ($m_oDatabase->checkPass($m_sUser, $cyrptpass) and $newpass eq $retry and $newpass =~ /^\S{6,50}$/) {
        $pwChanged = 1;
        my $md5 = new MD5;
        $md5->add($m_sUser);
        $md5->add($newpass);
    ...
    $m_sContent .= '<div class="showTables"><b>' . translate('done') . '</b>
    <br/>
    <table class="flat">
    <tr>
    ...
    $m_sContent .= '</table>'
      . a(
          {
           href  => "javascript:requestURI('$ENV{SCRIPT_NAME}?action=showProfile','showProfile','showProfile')",
           class => 'links'
          },
          translate('next')
      ).'</div>';
...
sub GetTypes
{
    ...
    $typId++;
    if (defined $type and defined $name) {#todo title setzen
